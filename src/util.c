...
util_resampler_type_to_str (int type)
{
    ...
    switch (type)
    {
        case SEQUENCE_SINC: strcpy (str, "sinc");
            break;
        case SEQUENCE_LINEAR: strcpy (str, "linear");
            break;
...
int
util_execvp (char *command, char *argv[])
{
...
char *
util_str_creplace (char *subject, char from, char to)
{
...
int
util_mkdir (const char *path, int mode)
{
#ifdef __WIN32__
    return (mkdir (path) == 0);
#else
...
static int
util_wipe_content (char *path, char *security_pattern)
{
    int success = 0;
