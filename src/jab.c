...
_jab_xml_scan (xmlDocPtr doc, char *xroot, char *xsub, void *content, int type)
{
  ...
  sprintf(xpath, "%s%s", xroot, xsub);
	...
	if (result != NULL) 
   {
    ...
    switch (type)
     {
      case JAB_XML_STRING : strcpy ((char *) content, str); break;
      case JAB_XML_FLOAT  : sscanf (str, "%f", (float *) content); break;                      
      case JAB_XML_INTEGER : sscanf (str, "%d", (int *) content); break;                      
     }
    xmlFree ((xmlChar *)str);
